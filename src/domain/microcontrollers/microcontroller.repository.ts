...
export async function getMicrocontrollerId(
  macAddress: string
  ...
  try {
    ...
    if (querySnapshot.empty) {
      throw new Error(
        `Microcontrolador com MAC ${macAddress} não está registrado`
      );
    }
...
export async function saveCoordinate(coordinate: Coordinate): Promise<void> {
  try {
    ...
    if (
      !coordinate.microcontroller_uid ||
      !coordinate.user_id ||
    ...
    const coordinateToSave = {
      ...coordinate,
      created_at:
        coordinate.created_at instanceof Date
          ? coordinate.created_at
          : new Date(coordinate.created_at),
    };

    const coordinatesRef = collection(db, "coordinates");
    await addDoc(coordinatesRef, coordinateToSave);
  } catch (error) {
